...
resource "aws_transfer_server" "sftp_server" {
  ...
  dynamic "endpoint_details" {
    ...
    content {
      ...
      vpc_id                 = var.vpc_id
      security_group_ids     = var.is_public && var.create_sftp_security_group ? compact(concat([aws_security_group.sftp[0].id], var.sftp_security_group_ids)) : var.sftp_security_group_ids
